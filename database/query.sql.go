...
const getFailedInLast24hr = `-- name: GetFailedInLast24hr :many
...
type GetFailedInLast24hrRow struct {
	Queue    string
	Cnt      int64
	ErrCount int64
}

func (q *Queries) GetFailedInLast24hr(ctx context.Context) ([]GetFailedInLast24hrRow, error) {
	rows, err := q.db.Query(ctx, getFailedInLast24hr)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetFailedInLast24hrRow
	for rows.Next() {
		var i GetFailedInLast24hrRow
		if err := rows.Scan(&i.Queue, &i.Cnt, &i.ErrCount); err != nil {
			return nil, err
		}
